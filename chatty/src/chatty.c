...
static void update_time(PblTm* t) {
  ...
  if (fuzzymin == 15 || fuzzymin == 45) {
  	...
  	case 35:
		...
		text_layer_set_font(&label, smallfont);
		break;
	}
  }
  ...
  if (strlen(buf) <= strlen("it's gone quarter past six")) {
...
void handle_init(AppContextRef ctx) {
  ...
  biggestfont = fonts_get_system_font(FONT_KEY_GOTHAM_30_BLACK);
  smallfont = fonts_get_system_font(FONT_KEY_GOTHAM_30_BLACK);
